...
import { ArrowLeft, User, Phone, MapPin, ChevronDown, Clock } from "lucide-react";
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-rose-50/50 to-orange-50/30">
      ...
      >
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex-1 text-center">
            <h1 className="text-xl lg:text-2xl font-bold tracking-tight">Booking</h1>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 lg:py-14 space-y-8 lg:space-y-10">
        ...
        <motion.div
          ...
          className="bg-white/95 backdrop-blur-xl rounded-2xl lg:rounded-3xl shadow-xl border border-white/50 overflow-hidden hover:shadow-2xl transition-all duration-500"
        >
          <motion.div
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <span className="font-semibold text-base lg:text-lg">📅 Schedule Appointment</span>
            </div>
            <motion.div
                ...
                <div>
                  ...
                  {currentTimeIndex >= 0 && (
                    <p className="text-xs text-slate-500 mt-2 text-center">
                      ⏰ Past slots unavailable (Current: {TIMES[currentTimeIndex] || 'Closed'})
                    </p>
                  )}
                </div>
              </div>
        ...
        >
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <span className="font-semibold text-base lg:text-lg">👤 Customer Details</span>
            </div>
            <motion.div
              ...
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-8 xl:gap-12">
                ...
                >
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 lg:gap-6">
                    <Field icon={<User size={18} />} label="Full Name *" name="name" value={form.name} onChange={handleChange} />
                    <Field icon={<Phone size={18} />} label="Mobile *" name="mobile" value={form.mobile} onChange={handleChange} />
                  </div>

                  <Field icon={<MapPin size={18} />} label="Full Address *" name="address" value={form.address} onChange={handleChange} />

                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 lg:gap-6">
                    <Field label="Door No" name="doorNo" value={form.doorNo} onChange={handleChange} />
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <Field label="Issue (Optional)" name="reason" value={form.reason} onChange={handleChange} />
                  </div>
                </motion.div>
                ...
                >
                  ...
                  <div className="space-y-3 text-slate-700">
                    <div className="flex justify-between py-1.5">
                      ...
                      <span className="font-semibold text-slate-900 text-base lg:text-lg">{service.service_type_name}</span>
                    </div>
                    <div className="flex justify-between py-1.5">
                      <span className="font-medium text-sm">Date</span>
                      <span className="font-semibold text-slate-900 text-base lg:text-lg">{date}</span>
                    </div>
                    <div className="flex justify-between py-1.5">
                      <span className="font-medium text-sm">Time</span>
                      <span className="font-semibold text-slate-900 text-base lg:text-lg">{time || "--:--"}</span>
                    </div>
                    <div className="pt-5 mt-6 border-t border-slate-200/50 pb-3">
                      <div className="flex justify-between items-baseline mb-1.5">
                        <span className="text-2xl lg:text-3xl font-bold text-[#C62828]">
                          ₹{service.service_charge}
                        </span>
                      </div>
                      <div className="text-xs font-semibold uppercase tracking-wide text-slate-500 bg-slate-100 px-3 py-1 rounded-full inline-block">
                        Service Charge
                      </div>
                    </div>
                  </div>
  ...
  return (
    <div>
      ...
      <div className="relative">
        ...
        <input
          {...props}
          className={`w-full px-4 lg:px-11 py-3.5 lg:py-4 rounded-xl lg:rounded-2xl border-2 border-slate-200 focus:ring-3 focus:ring-[#C62828]/20 focus:border-[#C62828] bg-white shadow-lg text-sm lg:text-base font-medium transition-all h-13 lg:h-14 ${
            icon ? "pl-13" : ""
